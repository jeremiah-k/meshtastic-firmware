...
void AdminModule::handleGetNodeRemoteHardwarePins(const meshtastic_MeshPacket &req)
{
    ...
    constexpr size_t maxResponsePins =
        sizeof(response.node_remote_hardware_pins) / sizeof(response.node_remote_hardware_pins[0]);
    ...
    for (uint8_t i = 0;
        ...
        response.node_remote_hardware_pins[response.node_remote_hardware_pins_count++] =
            devicestate.node_remote_hardware_pins[i];
    }
    for (uint8_t i = 0;
         i < moduleConfig.remote_hardware.available_pins_count && response.node_remote_hardware_pins_count < maxResponsePins; i++) {
